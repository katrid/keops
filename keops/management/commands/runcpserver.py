#!/usr/bin/env python
"""
Idea and code snippets borrowed from http://www.xhtml.net/scripts/Katrid-CherryPy-server-KatridCerise
...
Examples:
  ...
  Run a CherryPy server as a daemon and write the spawned PID in a file
    $ manage.py runcpserver daemonize=true pidfile=/var/run/katrid-cpserver.pid
...
def start_server(options):
    ...
    from katrid.core.handlers.wsgi import WSGIHandler
    server = Server(
...
def runcpserver(argset=[], **kwargs):
    ...
    if options['daemonize']:
        ...
        from katrid.utils.daemonize import become_daemon
        if options['workdir']:
